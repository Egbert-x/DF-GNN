...
cpnet_all = None
cpnet_simple = None
qc_default = [398, 38420, 119560, 134894, 147576, 263177, 302536]
ac_default = [101533, 115817, 210959]


def load_resources():
    global concept2id, id2concept, relation2id, id2relation, concept2name
    id2concept = [w.strip() for w in open('.././data/ddb/sem_cuis.txt')]
    ...
    with open("../../data/semmedVER43_2022_R_PREDICATION.csv", "r", encoding="gb18030", errors="ignore") as f:
        for row in csv.reader(f, skipinitialspace=True):
            ...
            concept2name[row[4]] = row[5]
            concept2name[row[8]] = row[9]
...
def generate_adj_data_from_grounded_concepts_umls__use_glove(grounded_path, cpnet_graph_path, cpnet_vocab_path, output_path, num_processes):
    ...
    qa_data = []
    statement_path = grounded_path.replace('grounded_md_051_smaller', 'statement')
    statement_path = statement_path.replace('grounded', 'statement')
    with open(grounded_path, 'r', encoding='utf-8') as fin_ground, open(statement_path, 'r', encoding='utf-8') as fin_state:
        ...
        for j, line in enumerate(lines_ground):
            dic = json.loads(line)
            # for c in dic['qc']:
            #     print("concept:", c, "id:", concept2id[c])
            q_ids = set(concept2id[c] for c in dic['qc'])
            # if not q_ids:
            #     # print("cui2id not have qid, i: ", i)
            #     q_ids = set(qc_default)
            a_ids = set(concept2id[c] for c in dic['ac'])
            # if not a_ids:
            #     #print("cui2id not have aid, i: ", i)
            #     a_ids = set(ac_default)
            # print(q_ids)
            # print(a_ids)
            q_ids = q_ids - a_ids
...
if __name__ == '__main__':
    ...
    #     grounded_path = f".././data/medqa_usmle/grounded/{fname}.grounded_md_051_smaller.jsonl"
    #     kg_path = f".././data/ddb/sem_800000.graph"
    #     kg_vocab_path = f".././data/ddb/sem_vocab_800000.txt"
    #     output_path = f".././data/medqa_usmle/graph_sem_800000/{fname}_dfgnn_smaller_800000.graph.adj.pk"
    #
    #     generate_adj_data_from_grounded_concepts_umls__use_glove(grounded_path, kg_path, kg_vocab_path, output_path, 10)
        # test
        for fname in ["dev", "test", "train"]:
            grounded_path = f".././data/medmcqa/grounded/{fname}.grounded_md_051_smaller_new.jsonl"
            kg_path = f".././data/ddb/sem.graph"
            kg_vocab_path = f".././data/ddb/sem_vocab.txt"
            output_path = f".././data/medmcqa/graph_sem/{fname}_dfgnn_smaller_new.graph.adj.pk"
