...
def load_data(args, devices, kg):
    ...
    if args.local_rank != -1 and one_process_at_a_time:
        for p_rank in range(args.world_size):
            ...
            dataset = data_utils.DFGNN_DataLoader(args, args.train_statements, args.train_adj,
...
def construct_model(args, kg, dataset):
    ...
    model = modeling_dfgnn.DFGNN(args, encoder_load_path, k=args.k, n_ntype=n_ntype, n_etype=n_etype, n_concept=concept_num,
...
def calc_loss_and_acc(logits, labels, loss_type, loss_func):
    if logits is None:
        ...
        if loss_type == 'margin_rank':
            ...
            loss = loss_func(logits + 1e-7, labels)
        bs = labels.size(0)
        loss *= bs
...
def decode(args, has_test_split, devices, kg):
    assert args.load_model_path is not None
    load_model_path = args.load_model_path
    print("loading from checkpoint: {}".format(load_model_path))
    checkpoint = torch.load(load_model_path, map_location='cpu')
    ...
    print("***** decoding *****")
    print(
        f'| dataset: {args.dataset} | num_dev: {dataset.dev_size()} | num_test: {dataset.test_size()} | save_dir: {args.save_dir} |')

    model.eval()

    for eval_set, filename in zip([dev_dataloader, test_dataloader], ['decode_dev.txt', 'decode_test.txt']):
        outputs = []
        with torch.no_grad():
            for qids, labels, *input_data in tqdm(eval_set):
                # print("len qids:", len(qids))
                logits, gnn_attn, gnn_edge = model.decode(*input_data)
                ...
                for i, (qid, label, pred) in enumerate(zip(qids, labels, predictions)):
                    ...
                    outputs.append('attention: {}'.format(gnn_attn))
                    outputs.append(('edge: {}'.format(gnn_edge)))
...
def main(args):
    ...
    if args.local_rank in [-1, 0]:
        ...
        wandb.init(project="DF", config=args, name=args.run_name, resume="allow", id=wandb_id, settings=wandb.Settings(start_method="fork"), mode=wandb_mode)
    ...
    if args.mode == 'train':
        ...
        decode(args, has_test_split, devices, kg)
    else:
...
if __name__ == '__main__':
    ...
    # Data
    parser.add_argument('--cpnet_vocab_path', default=f'../qagnn/{args.data_dir}/ddb/sem_vocab.txt')
    ...
    # parser.add_argument('--train_adj', default=f'../qagnn/{args.data_dir}/{args.dataset}/graph_sem_800000/train_dfgnn_smaller.graph.adj.pk', help="The path to the retrieved KG subgraphs of the training set.")
    # parser.add_argument('--dev_adj', default=f'../qagnn/{args.data_dir}/{args.dataset}/graph_sem_800000/dev_dfgnn_smaller.graph.adj.pk', help="The path to the retrieved KG subgraphs of the dev set.")
    # parser.add_argument('--test_adj', default=f'../qagnn/{args.data_dir}/{args.dataset}/graph_sem_800000/test_dfgnn_smaller.graph.adj.pk', help="The path to the retrieved KG subgraphs of the test set.")
    parser.add_argument('--train_adj', default=f'../qagnn/{args.data_dir}/{args.dataset}/graph_sem/train_dfgnn_smaller_new.graph.adj.pk', help="The path to the retrieved KG subgraphs of the training set.")
    parser.add_argument('--dev_adj', default=f'../qagnn/{args.data_dir}/{args.dataset}/graph_sem/dev_dfgnn_smaller_new.graph.adj.pk', help="The path to the retrieved KG subgraphs of the dev set.")
    parser.add_argument('--test_adj', default=f'../qagnn/{args.data_dir}/{args.dataset}/graph_sem/test_dfgnn_smaller_new.graph.adj.pk', help="The path to the retrieved KG subgraphs of the test set.")
    # parser.add_argument('--train_adj', default=f'../qagnn/data/{args.data_dir}/{args.dataset}/graph_ddb/train_dfgnn_new.graph.adj.pk', help="The path to the retrieved KG subgraphs of the training set.")
    # parser.add_argument('--dev_adj', default=f'../qagnn/data/{args.data_dir}/{args.dataset}/graph_ddb/dev_dfgnn_new.graph.adj.pk', help="The path to the retrieved KG subgraphs of the dev set.")
    # parser.add_argument('--test_adj', default=f'../qagnn/data/{args.data_dir}/{args.dataset}/graph_ddb/test_dfgnn_new.graph.adj.pk', help="The path to the retrieved KG subgraphs of the test set.")
