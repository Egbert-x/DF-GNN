...
MODEL_NAME_TO_CLASS[model_name] = 'bert'
model_name_base = 'michiyasunaga/BioLinkBERT-base'
MODEL_NAME_TO_CLASS[model_name_base] = 'bert'
model_name = 'michiyasunaga/BioLinkBERT-large'
MODEL_NAME_TO_CLASS[model_name] = 'bert'
model_name = './biomed_model/pretrained_models/BioLinkBERT-large'
MODEL_NAME_TO_CLASS[model_name] = 'bert'
...
class DFGNN_DataLoader(object):
    ...
    def load_resources(self, kg):
        ...
        if kg == "cpnet":
            ...
            with open(cpnet_vocab_path, "r", encoding="utf8") as fin:
                self.id2concept = [w.strip() for w in fin]
            self.concept2id = {w: i for i, w in enumerate(self.id2concept)}
            self.id2relation = ["process_of", "affects", "augments", "causes", "diagnoses", "interacts_with", "part_of",
                        "precedes", "predisposes", "produces", "isa"]
            # self.id2relation = ['belongs_to_the_category_of', 'is_a_category', 'may_cause', 'is_a_subtype_of', 'is_a_risk_factor_of', 'is_associated_with',
            #                     'may_contraindicate', 'interacts_with', 'belongs_to_the_drug_family_of', 'belongs_to_drug_super-family', 'is_a_vector_for',
            #                     'may_be_allelic_with', 'see_also', 'is_an_ingradient_of', 'may_treat']
        elif kg == "umls":
    ...
    def load_input_tensors(self, input_jsonl_path, max_seq_length, mode='eval'):
        ...
        # cache_path = input_jsonl_path + "-sl{}".format(max_seq_length) + (("-" + self.model_type) if self.model_type != "roberta" else "") + '-dfgnn.loaded_cache'
        cache_path = input_jsonl_path + "-sl{}".format(max_seq_length) + (("-" + self.model_type) if self.model_type != "roberta" else "") + '-dfgnn_new.loaded_cache'
        use_cache = True
    ...
    def load_sparse_adj_data_with_contextnode(self, adj_pk_path, max_node_num, concepts_by_sents_list, mode='eval'):
        ...
        # cache_path = adj_pk_path + "-nodenum{}".format(max_node_num) + ("-cntsall" if self.cxt_node_connects_all else "") + '-dfgnn_smaller_pretrain.loaded_cache'
        # cache_path = adj_pk_path + "-nodenum{}".format(max_node_num) + ("-cntsall" if self.cxt_node_connects_all else "") + '-dfgnn_smaller.loaded_cache'
        cache_path = adj_pk_path + "-nodenum{}".format(max_node_num) + ("-cntsall" if self.cxt_node_connects_all else "") + '-dfgnn_smaller_new.loaded_cache'
        # cache_path = adj_pk_path + "-nodenum{}".format(max_node_num) + ("-cntsall" if self.cxt_node_connects_all else "") + '-dfgnn.loaded_cache'
        # cache_path = adj_pk_path + "-nodenum{}".format(max_node_num) + ("-cntsall" if self.cxt_node_connects_all else "") + '-dfgnn_ddb.loaded_cache'
