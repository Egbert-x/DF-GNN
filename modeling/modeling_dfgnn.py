...
class DFGNN(nn.Module):
    ...
    def __init__(self, args={}, model_name="roberta-large", k=5, n_ntype=4, n_etype=38,
        ...
        self.lmgnn, self.loading_info = LMGNN.from_pretrained(model_name, output_hidden_states=True, output_loading_info=True,
                                                              args=args, model_name=model_name, k=k, n_ntype=n_ntype, n_etype=n_etype,
                                                              ...
                                                              freeze_ent_emb=freeze_ent_emb, init_range=init_range, ie_dim=ie_dim, info_exchange=info_exchange,
                                                              ie_layer_num=ie_layer_num,  sep_ie_layers=sep_ie_layers, layer_id=layer_id)

    def decode(self, *inputs):
        bs, nc = inputs[0].size(0), inputs[0].size(1)
        # print("bs:",bs)
        # print("nc:", nc)
        logits, _, _= self.forward(*inputs, cache_output=True)
        gnn_attn, gnn_edge = self.lmgnn.decode()
        # print("logits:", logits)
        # print("len gnn_attn:", len(gnn_attn), len(gnn_attn[0]))
        # print("len gnn_edge:", len(gnn_edge), len(gnn_edge[0][0]), len(gnn_edge[1][0]))
        # lm_attn = lm_attn.view(bs, nc, -1)
        # gnn_attn = gnn_attn.view(bs, nc)
        return logits, gnn_attn, gnn_edge
...
def test_DFGNN(device):
    cp_emb = torch.load("data/cpnet/cp_emb.pt")
    model = DFGNN(pretrained_concept_emb=cp_emb).to(device)
...
class LMGNN(PreTrainedModelClass):
    ...
    def decode(self):
        gnn_attn, gnn_edge = self.bert.decode()
        return gnn_attn, gnn_edge
    ...
    def forward(self, inputs, concept_ids, node_type_ids, node_scores, adj_lengths, special_nodes_mask, adj, lp_data, emb_data=None, cache_output=False):
        ...
        # Merged core
        lm_outputs, gnn_output = bert_or_roberta(input_ids, token_type_ids, attention_mask, output_mask, gnn_input, adj, node_type_ids, node_scores, special_nodes_mask, output_hidden_states=True, cache_output=cache_output)
...
class TextKGMessagePassing(ModelClass):
    ...
    def decode(self):
        gnn_attn = self.gnn_attn
        gnn_edge = self.gnn_edge
        return gnn_attn, gnn_edge
    ...
    def forward(self, input_ids, token_type_ids, attention_mask, special_tokens_mask, H, A, node_type, node_score, special_nodes_mask, cache_output=False, position_ids=None, head_mask=None, output_hidden_states=True):
        ...
        # GNN inputs
        _batch_size, _n_nodes = node_type.size() # _batch_size = 8
        ...
        # Merged core
        encoder_outputs, _X, gnn_detail = self.encoder(embedding_output,
                                       extended_attention_mask, special_tokens_mask, head_mask, _X, edge_index, edge_type, _node_type, _node_feature_extra, special_nodes_mask, output_hidden_states=output_hidden_states, cache_output=cache_output)

        #GNN attention weights and edge_index
        # gnn_attn = gnn_detail[0]
        # gnn_edge = gnn_detail[1]
        # print("gnn_attn len:", len(gnn_attn))
        # print("gnn_attn[0] len:", len(gnn_attn[0]))
        # print("gnn_edge len:", len(gnn_edge))
        # print("gnn_edge[0] len:", len(gnn_edge[0]))
        if cache_output:
            self.gnn_attn = gnn_detail[0]
            self.gnn_edge = gnn_detail[1]
...
class RoBERTaGAT(BertEncoder):
    ...
    def forward(self, hidden_states, attention_mask, special_tokens_mask, head_mask, _X, edge_index, edge_type, _node_type, _node_feature_extra, special_nodes_mask, output_attentions=False, output_hidden_states=True, cache_output=False):
        """
        hidden_states: [bs, seq_len, sent_dim]
        attention_mask: [bs, 1, 1, seq_len]
        head_mask: list of shape [num_hidden_layers]

        _X: [`total_n_nodes`, d_node] where `total_n_nodes` = b_size * n_node = 8 * 200   d_node = 200
        edge_index: [2, n_edges]
        edge_type: [n_edges]
        _node_type: [bs * n_nodes]
        _node_feature_extra: [bs * n_nodes, node_dim]  node_dim = 200
        """
        bs = hidden_states.size(0)  # bs = 8
        all_hidden_states = ()
        all_attentions = ()
        al_gnn_attn = []
        all_edge_map = []
        for i, layer_module in enumerate(self.layer):
            ...
            if i >= self.num_hidden_layers - self.k:
                ...
                if cache_output:
                    ...
                    al_gnn_attn.append(gnn_attn)
                    all_edge_map.append(edge_map)
                else:
                    _X = self.gnn_layers[gnn_layer_index](_X, edge_index, edge_type, _node_type,_node_feature_extra,return_attention_weights=cache_output)

                # _X, _X_attn = self.gnn_layers[gnn_layer_index](_X, edge_index, edge_type, _node_type, _node_feature_extra,return_attention_weights=True)
        ...
        return outputs, _X, (al_gnn_attn, all_edge_map) # last-layer hidden state, (all hidden states), (all attentions)
